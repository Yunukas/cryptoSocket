...
const api = () => {
	...
	//const subscription = ['0~Bittrex~BTC~USD'];
...
const displayData = (current, from, tsym, fsym) => {
	//console.log(current);
	...
	for (let key in current) {
		if (key === 'CHANGE24HOURPCT') {
			...
			return (CCC.convertValueToDisplay(tsym, current[key]));
		}
			...
			return (CCC.convertValueToDisplay(fsym, current[key]));
		}
		else {
			return (current[key]);
		}
...
CCC.TRADE.unpack = (tradeString) => {
    ...
    var unpackedTrade = {};
    var currentField = 0;
    for(var property in CCC.TRADE.FIELDS){
        if(CCC.TRADE.FIELDS[property] === 0)
        {
            unpackedTrade[property] = valuesArray[currentField];
            currentField++;
        }
        else if(maskInt&CCC.TRADE.FIELDS[property])
        {
            unpackedTrade[property] = valuesArray[currentField];
            currentField++;
        }
...
CCC.CURRENT.pack = (currentObject) =>
{
    var mask = 0;
    var packedCurrent ='';
			...
			//subscribing to trades as well in order to show the last market
         	if(property === 'LASTMARKET'){
                unpackedCurrent[property] = valuesArray[currentField];
				 ...
				 console.log(property + ":" + unpackedCurrent[property]);
            }
			currentField++;
        }
	}
...
CCC.CURRENT.getKey = (currentObject) => {
